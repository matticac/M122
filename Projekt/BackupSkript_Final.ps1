# ================================================================
# Office Backup System - Final Edition v5.5 (Log in Destination)
# ================================================================
# Change: Log-Datei wird nun im ZIEL-ORDNER (z.B. C:\Backup) gespeichert
# Feature: Slim Log (nur wichtige Infos)
# Feature: Alle Fixes (Space Check, Long Path, etc.)
# Autor: M122
# ================================================================
...
# NEU: Log-Pfad basiert jetzt auf dem Zielordner aus der Config
if ($script:BackupConfig) {
    $LogFile = Join-Path $script:BackupConfig.DestinationPath "backup_log.txt"
} else {
    $LogFile = Join-Path $BaseDir "backup_log.txt" # Fallback
}
...
function Write-Log {
    ...
    try {
        # Update Log Path falls Config geändert wurde
        if ($script:BackupConfig.DestinationPath) {
             $global:LogFile = Join-Path $script:BackupConfig.DestinationPath "backup_log.txt"
        }
...
function Save-Config {
    try { 
        $script:BackupConfig | ConvertTo-Json -Depth 5 | Out-File $ConfigFile -Encoding UTF8 -Force 
        Write-Log "Konfiguration gespeichert." "CONFIG"
    } catch {
        Write-Log "Fehler beim Speichern der Config: $_" "ERROR"
    }
}
...
function Check-DiskSpace {
    ...
    try {
        ...
        if ($drive.SizeRemaining -lt $RequiredBytes) {
            Write-Log "Speicherplatz-Warnung: Nur noch $($drive.SizeRemaining) Bytes frei." "WARN"
...
function Start-VmUpload {
    ...
    Write-Log "Starte Upload zu VM ($IP)..." "UPLOAD"
    if (-not (Test-Connection -ComputerName $IP -Count 1 -Quiet)) { 
        Write-Log "Upload fehlgeschlagen: VM Offline" "ERROR"
        return @{ Success=$false; Message="VM Offline" } 
    }
    try {
        ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$User@$IP" "mkdir -p $RemotePath"
        if ($LASTEXITCODE -ne 0) { 
            Write-Log "Upload fehlgeschlagen: SSH Login Fehler" "ERROR"
            return @{ Success=$false; Message="SSH Login fehlgeschlagen" } 
        }
        scp -r -p -o StrictHostKeyChecking=no "$LocalPath" "$($User)@$($IP):$RemotePath"
        if ($LASTEXITCODE -eq 0) { 
            Write-Log "Upload erfolgreich abgeschlossen." "SUCCESS"
            return @{ Success=$true; Message="Upload OK" } 
        }
        else { 
            Write-Log "Upload fehlgeschlagen: SCP Fehler" "ERROR"
            return @{ Success=$false; Message="SCP Fehler" } 
        }
    } catch { 
        Write-Log "Upload Exception: $_" "ERROR"
        return @{ Success=$false; Message="Fehler: $_" } 
    }
}

function Start-Cleanup {
    Write-Log "Starte Bereinigung..." "CLEANUP"
    $dst = $script:BackupConfig.DestinationPath
    if (-not (Test-Path $dst)) { 
        [System.Windows.Forms.MessageBox]::Show("Backup-Pfad nicht gefunden.", "Fehler"); return 
    }
    ...
    foreach ($item in $items) {
        ...
        if ($itemName -lt $cutoffName) { 
            try { 
                ...
                $delCount++ 
            } catch { Write-Log "Fehler beim Löschen von $($item.Name)" "ERROR" }
        }
    }
    $msgLokal = "Lokal bereinigt: $delCount Objekte gelöscht."
    Write-Log "Cleanup fertig. $delCount alte Backups gelöscht." "CLEANUP"
    ...
    if ($script:BackupConfig.UseVM -and $script:BackupConfig.VmIP) {
        ...
        if ($ans -eq "Yes") {
             ...
             try {
                ...
                Write-Log "VM Cleanup fertig. $remDeleted gelöscht." "CLEANUP"
             } catch { [System.Windows.Forms.MessageBox]::Show("Fehler VM: $_", "Fehler"); Write-Log "VM Cleanup Fehler: $_" "ERROR" }
        }
...
function Start-BackupProcess {
    ...
    Write-Log "========================================" "START"
    Write-Log "Backup gestartet. Modus: $BackupMode" "INFO"
    Write-Log "Ziel: $DestinationPath" "INFO"
    ...
    if ($BackupMode -eq "Smart") {
        ...
        $backupChain = Get-BackupChain -BackupRoot $DestinationPath
        if ($backupChain.Count -eq 0) { $BackupMode="Full"; Write-Log "Kein Full-Backup gefunden. Wechsel auf Full." "WARN" }
    }
    ...
    if ($list.Count -gt 0) {
        ...
        if (-not $spaceCheck.OK) {
            ...
            Write-Log "Abbruch: Nicht genug Speicher. Nötig: $mbNeeded MB, Frei: $mbFree MB" "ERROR"
    ...
    if($list.Count -gt 0){
        ...
        foreach($i in $list){
            try {
                ...
                Write-Log "Fehler bei Datei $($i.F.Name): $_" "ERROR"
            }
        }
        ...
        if ($Compress) {
            ...
            $zipSpace = Check-DiskSpace -DestPath $DestinationPath -RequiredBytes ($stats.SizeBytes / 2)
             if (-not $zipSpace.OK) { Write-Log "Kein Platz für ZIP." "ERROR"; return @{Success=$false; Message="Zu wenig Platz für ZIP Erstellung!"} }

            try {
                Write-Log "Erstelle ZIP-Archiv..." "ZIP"
                Compress-Archive -Path "$root\*" -DestinationPath $zipFile -CompressionLevel Optimal -Force
                Remove-Item $root -Recurse -Force
                Write-Log "ZIP erstellt: $zipFile" "SUCCESS"
                return @{Success=$true; Files=$stats.Copied; Errors=$stats.Errors; Path=$zipFile; Mode=$BackupMode}
            } catch {
                Write-Log "ZIP Fehler: $_" "ERROR"
                return @{Success=$false; Message="Fehler beim Zippen: $_"}
            }
        }

        Write-Log "Backup erfolgreich beendet. Dateien kopiert: $($stats.Copied). Fehler: $($stats.Errors)" "SUCCESS"
        return @{Success=$true; Files=$stats.Copied; Errors=$stats.Errors; Path=$root; Mode=$BackupMode}
    }
    Write-Log "Keine neuen Dateien zu sichern." "INFO"
...
function Show-MainGUI {
    ...
    $form.Text = "Office Backup System v5.5 (Log in Destination)"; $form.Size="800,850"; $form.StartPosition="CenterScreen"; $form.FormBorderStyle="FixedDialog"; $form.MaximizeBox=$false
