# ================================================================
# Office Backup System - Final Edition v4.5 (Popup Edition)
# ================================================================
# Feature: Windows Benachrichtigungen (statt Teams)
# Feature: Cleanup-Funktion & SSH Automatisierung
# GUI: Keine Teams-Felder mehr, nur pure Funktion
# Autor: M122
# ================================================================
...
$DefaultConfig = @{
    ...
    VmUser = ""
    VmPass = "" 
    VmDest = "/home/kali/Backup" # Korrigierter Standardpfad
}
...
function Show-Notification {
    ...
    # Icon Objekt erstellen
    $icon = New-Object System.Windows.Forms.NotifyIcon
    
    # Passendes System-Icon wählen
    if ($Type -eq "Error") { 
    ...
    # Popup anzeigen (für 3 Sekunden)
    $icon.ShowBalloonTip(3000, $Title, $Message, $tipIcon)
    
    # Kurz warten damit es sichtbar bleibt, dann aufräumen
    # (In einer GUI friert das kurz ein, aber für 1 Sekunde ist das okay)
...
function Start-Cleanup {
    ...
    $fulls = Get-ChildItem -Path $dst -Directory | Where-Object { $_.Name -match "_Full$" } | Sort-Object Name -Descending
    if (-not $fulls) { 
        [System.Windows.Forms.MessageBox]::Show("Kein Full-Backup gefunden. Abbruch.", "Info")
        return 
    }
    
    $cutoffName = $fulls[0].Name
    $localDirs = Get-ChildItem -Path $dst -Directory | Where-Object { $_.Name -match "^Backup_" }
    $delCount = 0
    
    foreach ($d in $localDirs) {
        if ($d.Name -lt $cutoffName) { 
            try { Remove-Item -Path $d.FullName -Recurse -Force -ErrorAction Stop; $delCount++ } catch {}
        }
    }
    
    $msgLokal = "Lokal bereinigt: $delCount Ordner gelöscht."
...
function Show-MainGUI {
    ...
    $form.Text = "Office Backup System v4.5"; $form.Size="800,850"; $form.StartPosition="CenterScreen"; $form.FormBorderStyle="FixedDialog"; $form.MaximizeBox=$false
    ...
    $btnRef = New-Object System.Windows.Forms.Button; $btnRef.Text="Aktualisieren"; $btnRef.Location="15,620"; $btnRef.Size="150,30"; $btnRef.Add_Click({$lstH.Items.Clear();if(Test-Path $txtDst.Text){Get-ChildItem $txtDst.Text -Dir|Where Name -match "^Backup_"|Sort Name -Desc|ForEach{$lstH.Items.Add($_.Name)}}})
    ...
    $lblClean = New-Object System.Windows.Forms.Label; $lblClean.Text="Löscht alle Backups, die älter sind als das letzte 'Full'-Backup."; $lblClean.Location="20,30"; $lblClean.Size="400,20"
    ...
    $btnStart.Add_Click({
        ...
        if ($res.Success) {
            ...
            # --- POPUP ANZEIGEN (FEHLER) ---
            Show-Notification -Title "Backup Fehlgeschlagen" -Message "Bitte Logs prüfen!" -Type "Error"
        }
    })
...
function Show-SetupWizard {
    ...
    $bs.Add_Click({
        $script:BackupConfig=@{SourcePaths=@($t1.Text);DestinationPath=$t2.Text;ExcludePaths=@("TBZ");FileTypes=@{Word=$true;Excel=$true;PowerPoint=$true;PDF=$true;Images=$false;All=$false};BackupMode="Smart";UseVM=$false;VmIP="";VmUser="";VmPass="";VmDest="/home/kali/Backup"}
        Save-Config
        $wiz.Close()
    })
